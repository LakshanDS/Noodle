...
- **Setup requirement** — the GitHub App needs the **Checks: Read & write** repository permission. Apps created through Noodle's config flow (`POST /api/github/create-app` manifest) are born with it; apps created before that, or configured by hand, need it added manually (App settings → Permissions & events → Checks: Read and write → accept the pending change under Installed GitHub Apps → restart Noodle, since cached installation tokens keep the old grant for up to 1h). The bot settings page validates this live: `GET /api/github/app-check-permission` reads the installation's actual permission grant via the App JWT (`appPermissions` on `AuthProvider`, `GithubAppAuth.getInstallationPermissions`) and the UI shows a ✓ card or a warning with the deep link to fix it.
