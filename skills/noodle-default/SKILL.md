...
- **Root cause over symptom** — when the root-cause fix is equally small.
  Otherwise ship the small safe fix and note the root cause in `finish_run`.
...
Call `finish_run` when done — required. Say what you changed, why, and what you
**deliberately didn't** do. Ideal: "Fixed X in one line; skipped Y — add when Z."
